...
BASE=F
colors=c("Yukpa"="#FD7F23","Bari"="orangered")
system("mkdir GeneSets")
for(pop in c("Yukpa","Bari")){
	for(grou in c(1,4)[1]){
  		for(fl in c(0,5,10)){
		...
		merged<-merge(merged,sens[,c("HGNC","pch")],by="HGNC")
 		...
 		print(table(merged$Color)) 
        	...
        	plo<-ggplot()+coord_cartesian(xlim=c(min,max),ylim=c(min,max))+
          		 ...
          		 labs(x="-log10(P-value) from mean")+labs(y="-log10(P-value) from median",title = ifelse(pop=="Bari","A. with Bari as reference in LRT","B. with Yukpa as reference in LRT"))+
